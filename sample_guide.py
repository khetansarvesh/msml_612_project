...
from model.DDPM_guide import DDPM_guide
...
def sample(opt):
    ...
    diff = DDPM_guide(nn_model=UViT(**opt.network),**opt.diffusion,device=device,drop_prob=0.1)
    diff.to(device)

    target = os.path.join(opt.save_dir, "ckpts", f"model_{ep}.pth")
    print0("loading model at", target)
    checkpoint = torch.load(target, map_location=device)
    ema = EMA(diff, beta=0.9999, update_after_step=0, update_every=1)
    ema.to(device)
